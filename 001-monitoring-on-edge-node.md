...
The stack has three nodes: Heimdall (edge/network), Allfather (primary application host), and Muninn (NAS/media). The observability stack — Prometheus and Uptime Kuma, plus the `cAdvisor`/`node_exporter` agents Prometheus scrapes — has to live *somewhere*.
...
Run the monitoring stack (Prometheus, Uptime Kuma, Dockge) on **Heimdall**, the edge node. Allfather and Muninn run lightweight exporters (`node_exporter`, `cAdvisor`); Prometheus on Heimdall scrapes them centrally.
...
- **Positive:** Centralized Prometheus gives one place to query cross-node metrics, and Uptime Kuma gives one availability view.
